...
var intersection = function intersection(ab, cd) {
  var _ab = _rollupPluginBabelHelpers.slicedToArray(ab, 2),
    a = _ab[0],
    b = _ab[1];
  var _cd = _rollupPluginBabelHelpers.slicedToArray(cd, 2),
    c = _cd[0],
    d = _cd[1];
  if (a.x - b.x === 0) {
    var ratio_c = Math.abs(a.x - c.x);
    var ratio_d = Math.abs(a.x - d.x);
    var _x = a.x;
  ...
  if (c.x - d.x === 0) {
    var ratio_a = Math.abs(c.x - a.x);
    var ratio_b = Math.abs(c.x - b.x);
    var _x2 = c.x;
